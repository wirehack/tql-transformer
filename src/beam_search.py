...
class SeqGenerator(object):
    def __init__(self,decode_step,model,src_mask,memory,eos_id,beam_size,max_seq_len=50,att=False,len_norm_factor=0.0,len_norm_const=5.,device_ids=None):
    ...
    def beam_search(self,init_input):
        ...
        out_wids,lprobs = self.decode_step(self.model,self.src_mask,self.memory,init_input,beam_size=self.beam_size)
...
#                 print lprobs[i][k]
        ...
        for _ in range(self.max_seq_len-1):
            ...
            out_wids,lprobs = self.decode_step(self.model,self.src_mask,self.memory,next_input,beam_size=self.beam_size)
...
#         print lprob.data[0]
        lprobs.append(lprob.data[0])
        out_wids.append(out_wid.data[0])
